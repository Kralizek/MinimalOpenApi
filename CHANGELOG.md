...
- **Can/do parser selection**: `IOpenApiParser` now has a `bool CanParse(string filePath, string content)` method. The generator iterates a registered `_parsers` list and calls the first parser that returns `true`. Implementations check the file extension and may peek the `openapi` version field in the raw content to accept only documents within their supported version range. This makes it straightforward to introduce a version-targeted parser (e.g. for a future OpenAPI 4.0) by prepending it to the list without modifying any existing parser.
