...
- **Spec publishing and serving**: `<OpenApi Publish="true" />` copies spec files to the build and publish output; `MapOpenApiSchemas()` registers `GET /.openapi/schemas/{version}/{name}.{ext}` endpoints at startup and returns an `OpenApiSchemaMapResult` containing an `OpenApiSchemaEndpoint` descriptor for each mapped file. Each descriptor exposes the resolved public path, schema name, version, a `HasOverride` flag, and the concrete `RouteHandlerBuilder` for further configuration. The optional `PublishPathOverride` item metadata lets consumers specify a custom HTTP route for a published spec file. The returned descriptors make it straightforward to wire up documentation UIs such as Swagger UI without duplicating path calculations.
